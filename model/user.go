...
func (options UserSortOptions) Apply(query *gorm.DB) *gorm.DB {
	...
	// 置顶用户永远排在最前，再套用请求的排序列
	q := query.Order(clause.OrderByColumn{
		Column: clause.Column{Name: "pinned"},
		Desc:   true,
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, positiveQuotaOnly bool, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
