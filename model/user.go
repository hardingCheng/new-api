...
func GetAllUsers(pageInfo *common.PageInfo) (users []*User, total int64, err error) {
	...
	err = tx.Unscoped().Order("pinned desc, id desc").Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password", "access_token").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, positiveQuotaOnly bool) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
	...
	// 获取分页数据
	err = query.Omit("password", "access_token").Order("pinned desc, id desc").Limit(num).Offset(startIdx).Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if err = tx.Model(&current).Omit("external_id", "quota", "used_quota", "request_count").Updates(newUser).Error; err != nil {
		return err
	}
