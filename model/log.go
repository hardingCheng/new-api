...
func formatUserLogs(logs []*Log, startIdx int) {
	for i := range logs {
		...
		if otherMap != nil {
			...
			// admin_info. User log APIs must remain safe for historical rows too.
			delete(otherMap, "user_pricing_overrides")
			delete(otherMap, "model_quota_pools")
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, usernames []string, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	tx := LOG_DB.Table("logs").Select(
		"COALESCE(sum(case when type = ? then quota else 0 end), 0) quota, COALESCE(sum(case when type = ? then quota else 0 end), 0) refund_quota",
