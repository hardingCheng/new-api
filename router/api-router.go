...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/pricing", middleware.HeaderNavModuleRequiredAuth("pricing"), controller.GetPricing)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			userRoute.GET("/cross_login", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.CrossLogin)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/:id/pricing_context", controller.GetUserPricingContext)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.GET("/channel_breaker/statuses", controller.GetChannelBreakerStatuses)
			optionRoute.GET("/channel_breaker/logs", controller.GetChannelBreakerLogs)
			optionRoute.POST("/channel_breaker/clear", controller.ClearChannelBreakerStatus)
		...
		// 对话采集查看页的入场票：后台只在 root 手动进入时换一张，票带着跳 /_dump/
		apiRouter.GET("/chatdump/viewer_ticket", middleware.RootAuth(), middleware.DisableCache(), ChatDumpViewerTicket)
		...
		logRoute.GET("/stat/breakdown", middleware.AdminAuth(), controller.GetLogsStatBreakdown)
