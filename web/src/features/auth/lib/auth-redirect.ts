...
/**
 * 账号归属其他站点时，后端不在本站建会话，只回一次性跳转地址。
 * 命中则把浏览器交给归属站点，返回 true 表示本次登录已经交接出去。
 */
export function handOffToOwningStation(payload: unknown): boolean {
  const target = (payload as { cross_login_url?: unknown } | null)
    ?.cross_login_url
  if (typeof target !== 'string' || target.trim() === '') return false

  let redirectURL: URL
  try {
    redirectURL = new URL(target, window.location.origin)
  } catch {
    return false
  }
  if (!allowedRedirectProtocols.has(redirectURL.protocol)) return false
