...
func setupLoginAtAuthVersion(user *model.User, expectedAuthVersion int64, c *gin.Context) {
	...
	// 跨站登录:账号归属其他站点时不在本站建会话,发一次性令牌跳回归属站点
...
func SearchUsers(c *gin.Context) {
	...
	group := c.Query("group")
	positiveQuotaOnly := c.Query("positive_quota") == "true"
	var role *int
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsers(keyword, group, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), positiveQuotaOnly, sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetUserPricingContext(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		common.ApiError(c, err)
		return
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		"data":    service.GetUserPricingContext(user),
	})
	return
}
