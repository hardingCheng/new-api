...
func buildTestLogOther(c *gin.Context, info *relaycommon.RelayInfo, priceData types.PriceData, usage *dto.Usage, tieredResult *billingexpr.TieredResult) map[string]interface{} {
	other := service.GenerateTextOtherInfo(c, info, priceData.ModelRatio, priceData.GroupRatioInfo.GroupRatio, priceData.CompletionRatio,
		usage.PromptTokensDetails.CachedTokens, priceData.CacheRatio, priceData.ModelPrice, priceData.GroupRatioInfo.EffectiveUserRatio())
	if tieredResult != nil {
...
func performChannelTests(ctx context.Context, channels []*model.Channel, testUserID int, allowDisable bool, report func(processed, total int)) channelTestSummary {
	...
	var disableThreshold = int64(common.GetChannelDisableThreshold() * 1000)
	...
	for index, channel := range channels {
		...
		// 当错误检查通过，才检查响应时间
		if common.IsAutomaticDisableChannelEnabled() && !shouldBanChannel {
		...
		if allowDisable && isChannelEnabled && shouldBanChannel && channel.GetAutoBan() && !exempt {
			processChannelError(result.context, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(result.context, constant.ContextKeyChannelKey), channel.GetAutoBan(), exempt), newAPIError)
			summary.Disabled++
		}
...
func retestRecoverableChannels(ctx context.Context) error {
	...
	if !retestDisabledChannelsLock.TryLock() {
		return errors.New("retest is already running")
	}
	defer retestDisabledChannelsLock.Unlock()
...
func AutomaticallyRetestDisabledChannels() {
	...
	autoRetestDisabledChannelsOnce.Do(func() {
		for {
			...
			if err := retestRecoverableChannels(context.Background()); err != nil {
