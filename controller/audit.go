...
var auditContentTemplates = map[string]string{
	...
	"user.quota_add":        "Increased user quota for ${target_username} (ID: ${target_user_id}) by ${quota}",
	"user.quota_subtract":   "Decreased user quota for ${target_username} (ID: ${target_user_id}) by ${quota}",
	"user.quota_override":   "Overrode user quota for ${target_username} (ID: ${target_user_id}) from ${from} to ${to}",
